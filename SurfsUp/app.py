...
def stations():
    stations_collection = session.query(Station.station).all()
...
def tobs():
    ...
    begin_date = dt.strptime(most_recent_date, '%Y-%m-%d').date() - dt.timedelta(days=365)
...
def start(start):
    ...
    calculation_results = calculate[0] if calculate else {"TMIN": None, "TAVG": None, "TMAX": None}
    print(calculate)
    print(type(calculation_results))
    results = list(np.ravel(calculate))
...
def start_end(start,end):
    ...
    if start_date > end_date:
        return jsonify({"error": "Start date must fall within range."}), 400
    ...
    #print(start_end)
    #print(type(start_end_results))
    results = list(np.ravel(start_end))
    return jsonify(results)
